...
func (h *Hub) Broadcast(message common.NetPacket, sender *webrtc.DataChannel) {
	h.mu.RLock()
	defer h.mu.RUnlock()

	// Get the sender's username
	senderUsername := h.usernames[sender]
	formattedMessage := common.NetPacket{
	...
	for channel := range h.connections {
		...
		go func(ch *webrtc.DataChannel, msg common.NetPacket) {
			...
			if err := ch.Send(buffer.Bytes()); err != nil {
...
func writeAnswer(res http.ResponseWriter, peerConnection *webrtc.PeerConnection, offer []byte, path string) {
	...
	peerConnection.OnDataChannel(func(dataChannel *webrtc.DataChannel) {
		...
		dataChannel.OnOpen(func() {
			...
			welcomeMessage := common.NetPacket{Username: username}
			var welcomeBuffer bytes.Buffer
			enc := gob.NewEncoder(&welcomeBuffer)
			if err := enc.Encode(welcomeMessage); err != nil {
				fmt.Printf("Failed to encode welcome message: %v\n", err)
				return
			}
			if err := dataChannel.Send(welcomeBuffer.Bytes()); err != nil {
		...
		dataChannel.OnMessage(func(msg webrtc.DataChannelMessage) {
			...
			//fmt.Printf("Message from DataChannel '%s': '%s'\n", dataChannel.Label(), packet)

			// broadcast the message to all other connected clients
			broadcastHub.Broadcast(packet, dataChannel)
		})
	})
